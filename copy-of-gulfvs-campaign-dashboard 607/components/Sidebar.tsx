...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, currentPage, onPageChange, onReset, user, onUserToggle, stats }) => {
  const menuItems = [
  ...
  const handleDownloadProjectData = () => {
    if (window.confirm("Sync local vault to data.json? This ensures your current state is preserved when sharing files.")) {
      db.downloadProjectJSON();
    }
  };

  return (
    <aside className={`
      fixed inset-y-0 left-0 w-72 bg-[#0f1c3a] text-white flex flex-col z-50 p-6 
      ...
      <nav className="space-y-2 shrink-0">
        {menuItems.map((item) => (
          <button
            ...
            className={`w-full group flex items-center space-x-4 px-5 py-4 rounded-2xl transition-all duration-300 ${
              currentPage === item.id 
      ...
      <div className="space-y-4 mt-auto pt-12 shrink-0">
        <div className="bg-white/5 rounded-3xl p-5 border border-white/5 backdrop-blur-md">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={onUserToggle} className="text-[10px] text-accent font-black hover:underline tracking-tight">SWITCH</button>
          </div>
        ...
        <button
          ...
          className="w-full py-4 bg-accent/10 border border-accent/20 rounded-2xl text-[10px] font-black text-accent uppercase tracking-widest hover:bg-accent hover:text-white transition-all transform active:scale-95"
        >
          💾 Synchronize Project
        </button>
