...
const ImportExportPage: React.FC<ImportExportProps> = ({ companies, logs }) => {
  ...
  // Helper to normalize data from various import formats (Title Case, camelCase, etc.)
  const normalizeCompanyData = (row: any): Omit<Company, 'id' | 'createdAt'> | null => {
    ...
    // Handle Interest Status logic (boolean or string)
    const interestedRaw = row['Interest Status'] || row['isInterested'] || row['Interested'] || row['Status'];
    const isInterested =
      interestedRaw === 'High Interest' ||
      interestedRaw === true ||
      String(interestedRaw).toLowerCase() === 'true';
  ...
  const handleExportExcel = () => {
    const data = getFilteredData();
    db.exportToExcel(data, `GulfVS_${source}_${timeframe}_${new Date().toISOString().split('T')[0]}`);
  };
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const mappedCompanies = await processInChunks(rawData, (row) => {
          return normalizeCompanyData(row);
        });
  ...
  const handleJsonPaste = async () => {
    ...
    try {
      ...
      const companies = raw
        .map((r: any) => normalizeCompanyData(r))
        .filter((c): c is Omit<Company, 'id' | 'createdAt'> => c !== null);

      if (companies.length === 0) throw new Error("No valid contacts found in JSON.");

      const result = await db.bulkAddCompanies(companies);
      setImportStatus(result);
      setJsonInput('');
      setTimeout(() => window.location.reload(), 1500);
    } catch (e: any) {
    ...
    <div className="space-y-8 md:space-y-12 animate-in fade-in duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900/50 p-8 rounded-[2.5rem] shadow-glass border border-slate-200/50 dark:border-slate-800 transition-all hover:shadow-xl hover:border-primary/20">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Ingestion Control Center</h3>
              <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
                <button onClick={() => setImportMode('file')} className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all ${importMode === 'file' ? 'bg-white dark:bg-slate-700 shadow-sm text-primary' : 'text-slate-400 hover:text-slate-600'}`}>File</button>
                <button onClick={() => setImportMode('json')} className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all ${importMode === 'json' ? 'bg-white dark:bg-slate-700 shadow-sm text-accent' : 'text-slate-400 hover:text-slate-600'}`}>JSON</button>
              </div>
            </div>
            ...
            {importMode === 'json' ? (
              <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                <textarea
                  ...
                  className="w-full h-32 bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl p-4 text-[10px] font-mono mb-4 focus:ring-2 focus:ring-accent outline-none text-slate-600 dark:text-slate-300 resize-none placeholder:text-slate-400"
                  placeholder='[{"companyName": "Acme", "emails": ["info@acme.com"]}]'
                />
                <button
                  onClick={handleJsonPaste}
                  disabled={isImporting || !jsonInput.trim()}
                  className="w-full py-4 bg-accent/10 border border-accent/20 text-accent rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-accent hover:text-white transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isImporting ? 'Processing...' : 'Parse & Save'}
                </button>
              </div>
            ) : (
              <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                <p className="text-xs text-slate-500 font-medium mb-6 leading-relaxed">Merge specific lead lists (CSV/Excel/JSON) securely.</p>
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-4 text-center">Status: Operational</p>
          </div>
